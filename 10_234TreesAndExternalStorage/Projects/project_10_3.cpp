...
class Tree234 {
        ...
        void preOrder(Node* localRoot) {
            if (localRoot != nullptr) {
                ...
                for (i = 0; i < numCount; i++) {
                    preOrder(localRoot->getChild(i));
                    cout << localRoot->getItem(i)->dData << " ";
                }
                preOrder(localRoot->getChild(i));
            }
        }
        void privateSort(Node* localRoot, vector<long>& tempArray) {
            if (localRoot != nullptr) {
                int numCount = localRoot->getNumItems();
                int i;
                for (i = 0; i < numCount; i++) {
                    privateSort(localRoot->getChild(i), tempArray);
                    tempArray.push_back(localRoot->getItem(i)->dData);
                }
                privateSort(localRoot->getChild(i), tempArray);
            }
        }
    public:
        ...
        vector<long> sort() {
            vector<long> tempArray;
            privateSort(root, tempArray);
            return tempArray;
        }
};
...
class Tree234App {
    public:
        static void main() {
            ...
            vector<long> array { 37, 34, 60, 30, 35, 48, 82, 5, 32,
                                 40, 53, 69, 98, 33, 50, 57, 86 };
            const int size = array.size();

            for (int i = 0; i < size; i++)
                theTree->insert(array[i]);

            // theTree->displayTree();
            vector<long> sortedArray = theTree->sort();
            for (int i = 0; i < size; i++)
                cout << sortedArray[i] << " ";
            cout << endl;
